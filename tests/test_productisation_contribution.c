...
int main(void)
{
    ...
    assert(strcmp(adoption->application_id, "org.umicom.tms") == 0);
    assert(umi_product_application_adoption_validate(adoption) ==
           UMI_STATUS_OK);
    assert(umi_tms_productisation_snapshot(&snapshot) ==
           UMI_STATUS_OK);
    ...
    assert(umi_tms_product_session_init(&session) ==
           UMI_STATUS_OK);
    ...
    assert(umi_tms_product_workspace_guide(&workspace_guide) ==
           UMI_STATUS_OK);
